...
par(mfrow=c(1,2) )
plot(Mun_uso, col = "transparent", border="transparent", ylim=c(7375000, 7400000), xlim=c(750000, 770000) )

flo <- Mun_uso@data$CLASSE_USO == "formaÃ§Ã£o florestal"
plot(Mun_uso[flo,], col = "dark green", border="transparent", add=TRUE)

urb<- Mun_uso@data$CLASSE_USO == "Ã¡rea edificada"
...
ant<- Mun_uso@data$CLASSE_USO == "Ã¡rea antropizada"
plot(Mun_uso[ant,], col = "orange", border="transparent", add=TRUE)

nfl<- Mun_uso@data$CLASSE_USO == "formaÃ§Ã£o nÃ£o florestal"
plot(Mun_uso[nfl,], col = "yellow", border="transparent", add=TRUE)

mag<- Mun_uso@data$CLASSE_USO == "Ã¡gua"
plot(Mun_uso[mag,], col = "blue", border="transparent", add=TRUE)

legend("bottomleft", c("Floresta", "Formação n Flor", "Silvicultura", "Edificacoes",
                       "Area Antropizada", "Massa d'água"), fill=c("dark green","yellow",
                       "purple", "red", "orange", "blue"), cex=0.8)
axis(1)
axis(2)

##############################################################################################################################################

plot(Mun_all, col = "transparent", border="transparent", ylim=c(7375000, 7400000), xlim=c(750000, 770000))

flo <- Mun_all@data$CLASSE_USO == "formaÃ§Ã£o florestal"
plot(Mun_all[flo,], col = "dark green", border="transparent", add=TRUE)

urb<- Mun_all@data$CLASSE_USO == "Ã¡rea edificada"
...
ant<- Mun_all@data$CLASSE_USO == "Ã¡rea antropizada"
plot(Mun_all[ant,], col = "orange", border="transparent", add=TRUE)

nfl<- Mun_all@data$CLASSE_USO == "formaÃ§Ã£o nÃ£o florestal"
plot(Mun_all[nfl,], col = "yellow", border="transparent", add=TRUE)

legend("bottomleft", c("Floresta", "Formação não florestal","Silvicultura",
...
legend("bottomleft", c("Fragmentos Florestais", "APPs"),
...
plot(Mun_app, col = "transparent", border="transparent", ylim=c(7375000, 7400000), xlim=c(750000, 770000))

app_cda_0_10 <- Mun_app@data$HIDRO == "curso d'Ã¡gua (0 - 10m)"
plot(Mun_app[app_cda_0_10,], col = "dark green", border="transparent", add=TRUE)

app_cda_10_50<- Mun_app@data$HIDRO == "curso d'Ã¡gua (10 - 50m)"
plot(Mun_app[app_cda_10_50,], col = "yellow", border="transparent", add=TRUE)

app_cda_50_200<- Mun_app@data$HIDRO == "curso d'Ã¡gua (50 - 200m)"
plot(Mun_app[app_cda_50_200,], col = "purple", border="transparent", add=TRUE)

mda<- Mun_app@data$HIDRO == "massa d'Ã¡gua"
...
legend("bottomleft", c("APP curso d'água 0-10m", "APP curso d'água 10-50m", 
...
a.cda_10<-a.col.df[a.col.df$HIDRO=="curso d'Ã¡gua (0 - 10m)",]
soma_cda_10<-rowsum(a.cda_10$areaHa, group=a.cda_10$CLASSE_USO)

a.cda_50<-a.col.df[a.col.df$HIDRO=="curso d'Ã¡gua (10 - 50m)",]
soma_cda_50<-rowsum(a.cda_50$areaHa, group=a.cda_50$CLASSE_USO)

a.cda_200<-a.col.df[a.col.df$HIDRO=="curso d'Ã¡gua (50 - 200m)",]
...
```
  ...
  * Corpos d'água 50 - 200m: O total de APP, dispostas em corpos d'água 50 - 200m, a ser restaurado em `r Nome_Mun` é de `r soma_cda_200[2,]+soma_cda_200[4,]` ha, o que corresponde a `r 100*(soma_cda_200[2,]+soma_cda_200[4,]/colSums(soma_cda_200))` % do total de APPs presentes apenas em corpos d'água 50 - 200m do município.
  
### Transição florestal

```{r, "carrega e classica os raster", echo=FALSE, warning=FALSE, results='hide'}
